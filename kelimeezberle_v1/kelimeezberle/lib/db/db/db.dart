...
class DB {
  ...
  Future<Lists> insertLists(Lists lists) async {
  ...
  //Liste ID sine göre kelimeleri getirme
  Future<List<Word>> readWordByList(int ?listID) async {
    final db = await instance.database;
    final orderBy = '${WordTableFields.id} ASC'; //küçükten büyüğe sırala
  ...
  Future <List<Map<String,Object?>>> readListsAll() async {
    ...
    await Future.forEach(lists, (element) async
    {
      ...
      //Kelime sayısı ve öğrenilmemiş kelimelerin bilgilerini çektik//
      ...
      Map<String,Object?> temp = Map.of(wordInfoByList[0]); //Hata aldığımız için sıfırıncı indexi tempe atadık. Map.of kopyalama.
  ...
  //Öğrenildi veya öğrenilmedi olarak işaretle.(Listlerim)
  ...
  Future<int> deleteListsAndWordByList(int id) async {
    ...
    if (result == 1) {
      await db.delete(tableNameLists,
          where: '${ListsTableFields.id} = ?', whereArgs: [id]);
    }
